...
class ProjectionPlotter:
    ...
    def initialize(self, x_data: np.array, y_data: np.array):
        ...
        kernel_config = BasicMatern52Config(input_dimension=self.input_dimension)
...
if __name__ == "__main__":
    kernel_config = BasicMatern52Config(input_dimension=2)
    model_config = GPModelFastConfig(kernel_config=kernel_config)
    oracle = GPOracle2D(kernel_config, 0.01)
    oracle.initialize(-5, 5, 50)
    x_data, y_data = oracle.get_random_data(1000)
    small_x_data, small_y_data = oracle.get_random_data(300)
    projection_plotter = ProjectionPlotter()
    projection_plotter.initialize(x_data, y_data)
    model = ModelFactory().build(model_config)
    model.infer(small_x_data, small_y_data)
    projection_plotter.set_line(1, np.array([1.0, 0.0]))
    projection_plotter.plot_prediction_at_dimension(model)
    projection_plotter.plot_closest_data_points(small_x_data, small_y_data, 5)
    # projection_plotter.plot_data_points(small_x_data,small_y_data,1)
    projection_plotter.show()
