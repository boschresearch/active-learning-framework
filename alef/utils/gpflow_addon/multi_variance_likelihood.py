...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 21 11:13:01 2021
...
class MultiGaussian(ScalarLikelihood):
    ...
    def __init__(self, variance=tf.constant([1.0]), variance_lower_bound=DEFAULT_VARIANCE_LOWER_BOUND, **kwargs):
        """
    ...
    def _conditional_variance(self, F):
        return tf.concat([tf.fill([*tf.shape(F)[:-1], 1], var) for var in self.variance.numpy()], axis=-1)

    def _predict_mean_and_var(self, Fmu, Fvar):
        return tf.identity(Fmu), Fvar + self.variance

    def _predict_log_density(self, Fmu, Fvar, Y):
        Y, valid_entries = self.__validate_matrix(Y)
