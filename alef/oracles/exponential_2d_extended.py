...
class Exponential2DExtended(Standard2DOracle):
    def __init__(self, observation_noise):
        super().__init__(observation_noise, -2.0, 5.0)

    def f(self, x1, x2):
        return x1 * np.exp(-1 * np.power(x1, 2.0) - np.power(x2, 2.0)) * 0.5

    def query(self, x, noisy=True, scale_factor=1.0):
        function_value = (self.f(x[0], x[1]) + self.f(x[0] - 3.0, x[1] - 3.0)) * scale_factor
        if noisy:
...
if __name__ == "__main__":
    function = Exponential2DExtended(0.01)
