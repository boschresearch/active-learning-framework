...
class GPModelCVOracle(BaseObjectOracle):
    def __init__(
        ...
        self.k = 4
        self.cv_metric = CV_METRIC.NEG_RMSE
        self.splitter = KFold(n_splits=self.k)
        if fast_inference:
            self.gp_model_config = GPModelWithNoisePriorConfig(
                kernel_config=BaseKernelConfig(name="dummy", input_dimension=0), n_starts_for_multistart_opt=3
            )
        else:
            self.gp_model_config = GPModelWithNoisePriorConfig(
                kernel_config=BaseKernelConfig(name="dummy", input_dimension=0), n_starts_for_multistart_opt=5
            )

        if fast_inference:
            self.prediction_model_config = GPModelWithNoisePriorConfig(
                kernel_config=BaseKernelConfig(name="dummy", input_dimension=0), n_starts_for_multistart_opt=3
            )
        else:
            self.prediction_model_config = GPModelWithNoisePriorConfig(
    ...
    def query(self, x: Union[gpflow.kernels.Kernel, BaseKernelGrammarExpression]) -> np.float:
        ...
        for train_index, val_index in self.splitter.split(self.x_data):
            ...
            if self.cv_metric == CV_METRIC.LL:
                val_loglikeli = np.mean(model.predictive_log_likelihood(x_val_j, y_val_j))
                val_metrics.append(val_loglikeli)
            elif self.cv_metric == CV_METRIC.NEG_RMSE:
                pred_mu, _ = model.predictive_dist(x_val_j)
                rmse = calculate_rmse(pred_mu, y_val_j)
                val_metrics.append(-1 * rmse)
        cv_metric = np.mean(val_metrics)
        time_after_query = time.perf_counter()
        duration = time_after_query - time_before_query
        return cv_metric, duration
