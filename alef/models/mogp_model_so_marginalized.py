...
class SOMOGPModelMarginalized(BaseModel):
    ...
    def draw_from_hyperparameter_prior(self, show_draw: bool = True):
        ...
        for parameter in self.model.trainable_parameters:
            try:
                ...
                print(f"{parameter.prior.name} give invalid sample {new_value}, skip initial drawing")
        if show_draw:
            self.print_model_summary()
    ...
    def predict(self, x_test: np.array, prediction_quantity: PredictionQuantity) -> Tuple[np.array, np.array]:
        ...
        Arguments:
        x_test: Array of test input points with shape (n,d+1) where d is the input dimension and n the number of test points
