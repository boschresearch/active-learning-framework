...
import numpy as np
from alef.models.amortized_infer_structured_kernels.gp.base_kernels import BaseKernelEvalMode, BaseKernelTypes
...
class GPModelAmortizedStructured(BaseModel):
    ...
    def load_amortized_model(
        self,
        snapshot_file_path: str,
        is_checkpoint: bool = False,
        assert_model_config_dict: bool = False,
        set_to_eval_mode: bool = True,
    ):
        if is_checkpoint:
            checkpoint = torch.load(snapshot_file_path, map_location=self.device)
            model_state_dict = checkpoint["model"]
            if assert_model_config_dict:
                model_config_dict = checkpoint["model_config_dict"]
                assert model_config_dict == self.model_config.dict()
        else:
            model_state_dict = torch.load(snapshot_file_path, map_location=self.device)
