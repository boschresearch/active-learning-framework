...
class GPModelPytorch(BaseModel):
    ...
    def infer(self, x_data: np.array, y_data: np.array):
        x_data = torch.from_numpy(x_data).to(torch.get_default_dtype())
        y_data = torch.from_numpy(np.squeeze(y_data)).to(torch.get_default_dtype())
            ...
            if self.do_multi_start_optimization:
                ...
                for i in range(0, self.n_restarts_multistart):
                    try:
                        ...
                        print("Error occured in optimization - skip step")
    ...
    def predictive_dist(self, x_test: np.array) -> Tuple[np.array, np.array]:
        x_test = torch.from_numpy(x_test).to(torch.get_default_dtype())
        ...
        # mean, std = pred_dist.mean.numpy(), pred_dist.stddev.detach().numpy()
