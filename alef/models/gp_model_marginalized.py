...
class GPModelMarginalized(BaseModel, BatchModelInterace, BayesianEnsembleInterface):
    ...
    def plot_unnormalized_log_posterior_landscape(
        self,
        ...
        for i in range(0, n_grid):
            ...
            f = copied_model.log_posterior_density()
            x_values.append(np.array([transform_1(x1), transform_2(x2)]))
            f_values.append(f)
        plotter = Plotter2D(1)
        x_values = np.squeeze(np.array(x_values))
        f_values = np.array(f_values)
        plotter.add_gt_function(x_values, f_values, "viridis", 100, 0)
        x_label = self.param_to_name[self.model.trainable_parameters[0]]
        y_label = self.param_to_name[self.model.trainable_parameters[1]]
        plotter.configure_axes(0, 0, "(unnormalized) log posterior", x_label, y_label)
        if return_plotter:
            return plotter
        else:
            plotter.show()
    ...
    def predict_full_cov(self, x_test: np.array, prediction_quantity: PredictionQuantity) -> Tuple[np.array, np.array]:
        ...
        for i in range(0, self.num_samples):
            for var, var_sample in zip(self.hmc_helper.current_state, self.samples):
                var.assign(var_sample[i])
            if prediction_quantity == PredictionQuantity.PREDICT_F:
                pred_mus, pred_covs = self.model.predict_f(x_test, full_cov=True)
            elif prediction_quantity == PredictionQuantity.PREDICT_Y:
                pred_mus, pred_covs = self.model.predict_y(x_test, full_cov=True)
    ...
    def entropy_predictive_dist_full_cov(self, x_test: np.array) -> np.float:
        ...
        gmm = GaussianMixtureDensityNd(np.repeat(1 / n_samples, n_samples), pred_mus, pred_sigmas)
    ...
    def hyper_batch_bald(self, x_test: np.array) -> np.float:
        ...
        for i in range(0, n_samples):
            entropy_for_sample = multivariate_normal(pred_mus[i], pred_sigmas[i]).entropy()
            entropies_sample.append(entropy_for_sample)
        average_entropy = np.mean(entropies_sample)
        hyper_batch_bald = entropy - average_entropy
        return hyper_batch_bald
