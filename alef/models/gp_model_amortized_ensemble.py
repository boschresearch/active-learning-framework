...
from alef.models.amortized_infer_structured_kernels.gp.base_kernels import BaseKernelTypes
...
class GPModelAmortizedEnsemble(BaseModel):
    ...
    def load_amortized_model(self, snapshot_file_path: str, is_checkpoint: bool = False):
        model_config = self.model_config
        if is_checkpoint:
            checkpoint = torch.load(snapshot_file_path, map_location=self.device)
            model_state_dict = checkpoint["model"]
            checkpoint["model_config_dict"]
            # assert model_config_dict == model_config.dict()
        else:
