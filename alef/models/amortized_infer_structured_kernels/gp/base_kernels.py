...
from alef.configs.kernels.rbf_configs import RBFWithPriorConfig, BasicRBFConfig
from alef.configs.kernels.linear_configs import LinearWithPriorConfig, BasicLinearConfig
from alef.configs.kernels.periodic_configs import PeriodicWithPriorConfig, BasicPeriodicConfig
...
from alef.models.amortized_infer_structured_kernels.nn.module import MLP
from alef.utils.utils import get_gpytorch_kernel_from_expression_and_state_dict
...
from alef.models.amortized_infer_structured_kernels.gp.base_symbols import BaseKernelTypes, N_BASE_KERNEL_TYPES
...
class SEKernel(nn.Module, BaseKernel):
    def __init__(
        self, kernel_embedding_dim, dim_hidden_layer_list, dropout_p, eval_mode=BaseKernelEvalMode.STANDARD
    ) -> None:
        super().__init__()
        self.num_params = 2
        ...
        self.mlp = MLP(
            kernel_embedding_dim, dim_hidden_layer_list, self.num_params, dropout_p=dropout_p, use_biases=True
        )
        self.softplus = torch.nn.Softplus()
        self.lengthscale_index = 0
        self.variance_index = 1
        self.eval_mode = eval_mode
        self.debug_lengthscale = torch.tensor(BASE_KERNEL_LENGTHSCALE)
        self.debug_variance = torch.tensor(BASE_KERNEL_VARIANCE)
...
class MaternKernel(nn.Module, BaseKernel):
    def __init__(
        self, kernel_embedding_dim, dim_hidden_layer_list, dropout_p, nu: float, eval_mode=BaseKernelEvalMode.STANDARD
    ) -> None:
        super().__init__()
...
class PeriodicKernel(nn.Module, BaseKernel):
    def __init__(
        ...
        self.debug_lengthscale = torch.tensor(BASE_KERNEL_LENGTHSCALE)
        self.debug_variance = torch.tensor(BASE_KERNEL_VARIANCE)
        self.debug_period = torch.tensor(BASE_KERNEL_PERIOD)
...
class LinearKernel(nn.Module, BaseKernel):
    def __init__(
        ...
        self.variance_index = 1
        self.eval_mode = eval_mode
        self.debug_offset = torch.tensor(BASE_LINEAR_KERNEL_OFFSET)
        self.debug_variance = torch.tensor(BASE_KERNEL_VARIANCE)
...
class KernelAddition(nn.Module, BaseKernel):
    ...
    def forward(self, x1, x2, kernel_embedding, untransformed_params=None):
        K = self.kernel1.forward(
            x1, x2, kernel_embedding, self.eval_param_list(untransformed_params, 0)
        ) + self.kernel2.forward(x1, x2, kernel_embedding, self.eval_param_list(untransformed_params, 1))
        return K

    def get_log_prior_prob(
        self, kernel_embedding: torch.tensor, untransformed_params: Optional[BaseKernelParameterFormat] = None
    ) -> torch.tensor:
        log_prior_prob_k1 = self.kernel1.get_log_prior_prob(
            kernel_embedding, self.eval_param_list(untransformed_params, 0)
        )
        log_prior_prob_k2 = self.kernel2.get_log_prior_prob(
            kernel_embedding, self.eval_param_list(untransformed_params, 1)
        )
    ...
    def get_untransformed_parameters(self, kernel_embedding):
        return [
            self.kernel1.get_untransformed_parameters(kernel_embedding),
            self.kernel2.get_untransformed_parameters(kernel_embedding),
        ]

    def get_parameters(self, kernel_embedding, detach: bool = True):
        return [
            self.kernel1.get_parameters(kernel_embedding, detach),
            self.kernel2.get_parameters(kernel_embedding, detach),
        ]
...
class BatchedDimWiseAdditiveKernelWrapper(nn.Module):
        ...
        K_batch = torch.stack(
            [
                self.dim_wise_additive_kernel_wrapper.forward(
                    ...
                    kernel_embeddings[b],
                    kernel_type_list[b],
                    self.eval_param_list(untransformed_params, b),
                )
    ...
    def get_num_params(self, kernel_type_list: List[KernelTypeList]) -> List[int]:
        num_params = [
            self.dim_wise_additive_kernel_wrapper.get_num_params(kernel_list) for kernel_list in kernel_type_list
        ]
        return num_params
    ...
    def get_untransformed_parameters(self, kernel_embeddings: torch.tensor, kernel_type_list: List[KernelTypeList]):
        ...
        untransformed_params_over_batch = [
            self.dim_wise_additive_kernel_wrapper.get_untransformed_parameters(
                kernel_embeddings[b], kernel_type_list[b]
            )
            for b in range(batch_size)
        ]
    ...
    def get_parameters(
        self, kernel_embeddings: torch.tensor, kernel_type_list: List[KernelTypeList], detach: bool = True
    ):
        batch_size = kernel_embeddings.size(0)
        params_over_batch = [
...
def create_kernel_embedding_tensor(kernel_list: List[BaseKernelTypes]) -> np.array:
    ...
    embedding_tensor = np.stack(
        [assign(np.zeros(N_BASE_KERNEL_TYPES, dtype=np.float32), kernel_type, 1.0) for kernel_type in kernel_list]
    )
...
def transform_kernel_list_to_expression(
    kernel_list: KernelTypeList, add_prior=True, use_gpflow=False, make_deep_copy=True
) -> BaseKernelGrammarExpression:
    """
    ...
    """
    final_expression, _ = get_expressions_from_kernel_list(kernel_list, add_prior, use_gpflow, make_deep_copy)
    return final_expression


def get_expressions_from_kernel_list(kernel_list, add_prior=True, use_gpflow=False, make_deep_copy=True):
    ...
    for d in range(0, num_dimension):
        ...
        for kernel_type in kernel_list[d]:
            if use_gpflow:
                expression = get_kernel_expression_for_base_kernel_type_gpflow(kernel_type, num_dimension, d, add_prior)
            else:
    ...
    final_expression = KernelGrammarExpressionTransformer.multiply_expressions(
        kernel_expression_list, make_deep_copy=make_deep_copy
    )
    return final_expression, single_expression_list


def get_parameter_value_lists(
    kernel_list: KernelTypeList, kernel_state_dict, wrap_in_addition: bool = True
    ...
    kernel_expression, single_expression_list = get_expressions_from_kernel_list(
        kernel_list, add_prior=True, make_deep_copy=False
    )
    ...
    get_gpytorch_kernel_from_expression_and_state_dict(
        kernel_expression, kernel_state_dict, wrap_in_addition=wrap_in_addition
    )
...
def get_kernel_expression_for_base_kernel_type_gpflow(
    base_kernel_type: BaseKernelTypes, input_dimension: int, active_dimension: int, with_prior: bool = True
) -> BaseKernelGrammarExpression:
    if with_prior:
        se_config = RBFWithPriorConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        lin_config = LinearWithPriorConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        per_config = PeriodicWithPriorConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        matern52_config = Matern52WithPriorConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
    else:
        se_config = BasicRBFConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        lin_config = BasicLinearConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        per_config = BasicPeriodicConfig(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
        matern52_config = BasicMatern52Config(
            input_dimension=input_dimension, active_on_single_dimension=True, active_dimension=active_dimension
        )
    if base_kernel_type == BaseKernelTypes.SE:
        expression = ElementaryKernelGrammarExpression(KernelFactory.build(se_config))
    elif base_kernel_type == BaseKernelTypes.LIN:
        expression = ElementaryKernelGrammarExpression(KernelFactory.build(lin_config))
    elif base_kernel_type == BaseKernelTypes.PER:
        expression = ElementaryKernelGrammarExpression(KernelFactory.build(per_config))
    elif base_kernel_type == BaseKernelTypes.MATERN52:
        expression = ElementaryKernelGrammarExpression(KernelFactory.build(matern52_config))
    elif base_kernel_type == BaseKernelTypes.LIN_MULT_PER:
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(lin_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(per_config))
        expression = KernelGrammarExpression(expression1, expression2, KernelGrammarOperator.MULTIPLY)
    elif base_kernel_type == BaseKernelTypes.SE_MULT_LIN:
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(se_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(lin_config))
        expression = KernelGrammarExpression(expression1, expression2, KernelGrammarOperator.MULTIPLY)
    elif base_kernel_type == BaseKernelTypes.SE_MULT_PER:
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(se_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(per_config))
        ...
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(se_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(matern52_config))
        expression = KernelGrammarExpression(expression1, expression2, KernelGrammarOperator.MULTIPLY)
    elif base_kernel_type == BaseKernelTypes.LIN_MULT_MATERN52:
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(lin_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(matern52_config))
        ...
        expression1 = ElementaryKernelGrammarExpression(KernelFactory.build(per_config))
        expression2 = ElementaryKernelGrammarExpression(KernelFactory.build(matern52_config))
        expression = KernelGrammarExpression(expression1, expression2, KernelGrammarOperator.MULTIPLY)
    else:
        raise NotImplementedError
...
def get_batch_from_nested_parameter_list(
    ...
    flattened_tensor_list = [
        torch.concat(flatten(kernel_parameter_list)) for kernel_parameter_list in kernel_parameter_lists
    ]
...
def flatten(S):
    ...
    while stack:
        ...
        if isinstance(current, list):
            stack.extend(current)  # Add elements in reverse order to maintain correct order
        else:
