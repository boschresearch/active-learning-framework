...
from alef.configs.base_parameters import BASE_KERNEL_LENGTHSCALE
from alef.configs.prior_parameters import KERNEL_LENGTHSCALE_GAMMA, KERNEL_VARIANCE_GAMMA
    ...
    def forward(
        ...
        for d in range(0, D):
            kernel_gram_at_d = self.se_kernel.forward(
                X1[:, d].unsqueeze(-1),
                X2[:, d].unsqueeze(-1),
                kernel_embeddings[d, 0, :],
                self.eval_param_list(untransformed_params, d, 0),
            )
            kernel_grams.append(kernel_gram_at_d)
        global_kernel_embedding = self.get_global_kernel_embedding(kernel_embeddings, D)
        constant = self.const_kernel.forward(
            X1, X2, global_kernel_embedding, self.eval_param_list(untransformed_params, d + 1, 0)
        )
    ...
    def get_untransformed_parameters(self, kernel_embeddings: torch.tensor, kernel_type_list: KernelTypeList):
        ...
        for d in range(0, D):
            untransformed_kernel_parms_per_dim = [
                self.se_kernel.get_untransformed_parameters(kernel_embeddings[d, 0, :])
            ]
...
class BatchedRBFKernelWrapper(BatchedDimWiseAdditiveKernelWrapper):
    ...
    def __init__(
        ...
        self.eval_mode = eval_mode
        self.dim_wise_additive_kernel_wrapper = RBFKernelWrapper(
            kernel_embedding_dim, dim_hidden_layer_list, dropout_p, eval_mode
        )
