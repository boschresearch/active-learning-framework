...
from alef.configs.prior_parameters import PRIOR_SETTING
...
from alef.models.amortized_infer_structured_kernels.config.training.training_configs import (
    BasicDimWiseAdditiveKernelTrainingConfig,
)
from alef.models.amortized_infer_structured_kernels.data_generators.dataset_of_datasets import (
    DatasetOfDatasets,
    DatasetType,
...
from alef.models.amortized_infer_structured_kernels.data_generators.simulator import SimulatedDataset
from alef.models.amortized_infer_structured_kernels.eval.evaluator import Evaluator
...
from alef.models.amortized_infer_structured_kernels.nn.amortized_models_factory import AmortizedModelsFactory
import torch
from torch.utils.data import DataLoader
from tensorboardX import SummaryWriter
import logging
import numpy as np
from alef.models.amortized_infer_structured_kernels.utils.enums import LossFunctionType, OptimizerType
from alef.models.amortized_infer_structured_kernels.utils.utils import default_worker_init_function, plot_predictions
...
class CheckpointStrategy(Enum):
    RESUME_COMPLETE = 1
    RESUME_MODEL_AND_OPT = 2
    RESUME_MODEL = 3


class TrainerProductKernels:
    def __init__(
        self,
        ...
        real_world_data_evaluator: Evaluator,
        run_suffix: str = "",
        ...
        self.do_real_world_dataset_eval = True
        ...
        self.activate_eval_mode_for_validation = True
        ...
        self.steps_per_epoch = int(len(self.train_dataset) / self.batch_size) + (
            len(self.train_dataset) % self.batch_size > 0
        )
        print("Steps per epoch: " + str(self.steps_per_epoch))
        self.val_epochs = 20
        ...
        # real world data evaluator
        self.real_world_data_evaluator = real_world_data_evaluator

    def write_configs_to_tensorboard(
        self, model_config, training_config, generate_dataset_on_the_fly, generator_config
    ):
    ...
    def train(self, use_gpu: bool, use_checkpoint: bool = False, checkpoint_file_path: str = None):
        ...
        print("Initial seed at beginning: " + str(torch.initial_seed()))
        ...
        optimizer, scheduler = self.initialize_optimizer_and_scheduler(model)
        self.global_step = 0
        ...
        # set new seed for training - previous seed was for eval dataset generation
        self.set_initial_seed(self.initial_seed * 2 + epoch_start_index)
        print("Initial seed before data loader: " + str(torch.initial_seed()))

        train_loader, val_loader, test_loader = self.initialize_data_loader()

        if self.do_real_world_dataset_eval:
            ####### Initialize real world data evalutor ###########
            self.evaluate_on_real_world_datasets(device, model, eval_with_gps=True, index=epoch_start_index)

        for epoch_index in range(epoch_start_index, self.num_epochs):
            ################## Validation #########################
            if epoch_index % self.val_epochs == 0:
                if self.activate_eval_mode_for_validation:
                    model.eval()

                if evaluate_on_train_datasets:  # it does not make sense to distinguish between train and val set if train set is resampled in each epoch anyway and the distributions are the same
                    # calculate complete loss on train set
                    # add 1D train prediction plots
                    self.add_prediction_plots(
                        device,
                        model,
                        evaluation_datasets_dict_train[1],
                        evaluation_datasets_ml_param_train[1],
                        epoch_index,
                        "train",
                    )
                    ...
                    if 2 in evaluation_datasets_dict_train:
                        self.add_prediction_plots(
                            device,
                            model,
                            evaluation_datasets_dict_train[2],
                            evaluation_datasets_ml_param_train[2],
                            epoch_index,
                            "train",
                        )
                    ...
                    self.tensorboard_writer.add_scalar(
                        "train_nmll_complete", train_mean_nmll_complete, global_step=epoch_index
                    )
                    self.tensorboard_writer.add_scalar(
                        "train_loss_complete", train_global_loss, global_step=epoch_index
                    )
                ...
                self.add_prediction_plots(
                    device,
                    model,
                    evaluation_datasets_dict_val[1],
                    evaluation_datasets_ml_param_val[1],
                    epoch_index,
                    "val",
                )
                ...
                if 2 in evaluation_datasets_dict_val:
                    self.add_prediction_plots(
                        device,
                        model,
                        evaluation_datasets_dict_val[2],
                        evaluation_datasets_ml_param_val[2],
                        epoch_index,
                        "val",
                    )

                # calculate complete loss on val set
                val_mean_nmll_complete, _, val_global_loss = self.calculate_full_dataset_loss(
                    model, val_loader, epoch_index, device
                )
                ...
                if self.do_real_world_dataset_eval:
                    # Evaluate model on real world datasets
                    self.evaluate_on_real_world_datasets(device, model, eval_with_gps=False, index=epoch_index)
            ...
            for batch_index, batch in enumerate(train_loader):
                ...
                if (
                    ...
                    pred_kernel_parameter_list = model.get_parameter_nested_lists(
                        kernel_embeddings_out, kernel_list, detach=False
                    )
                ...
                if nmll_success:
                    ...
                    self.tensorboard_writer.add_scalar(
                        "mean_nmlls_train", mean_nmlls_train, global_step=self.global_step
                    )
    ...
    def evaluate_on_real_world_datasets(self, device, model, eval_with_gps, index):
        self.load_model_to_evaluator(device, model)
        if eval_with_gps:
            self.real_world_data_evaluator.eval_model_with_tag(self.real_world_data_evaluator.gp_model_tag)
            self.real_world_data_evaluator.eval_model_with_tag(self.real_world_data_evaluator.gp_model_n_iter_tag)
            # self.real_world_data_evaluator.eval_model_with_tag(self.real_world_data_evaluator.svgp_model_tag)
        self.real_world_data_evaluator.eval_model_with_tag(self.real_world_data_evaluator.gp_amor_tag)
        self.real_world_data_evaluator.eval_model_with_tag(self.real_world_data_evaluator.gp_amor_warm_start_tag)
        fig_rmse, _, _ = self.real_world_data_evaluator.create_plots(True)
        self.tensorboard_writer.add_figure("RMSE_on_datasets", fig_rmse, global_step=index)
        # self.tensorboard_writer.add_figure("Time_on_datasets", fig_time, global_step=index)
        plt.close()
        _, fig_nll, _ = self.real_world_data_evaluator.create_plots(True, return_nll=True)
        self.tensorboard_writer.add_figure("NLL_on_datasets", fig_nll, global_step=index)
        # self.tensorboard_writer.add_figure("Time_on_datasets", fig_time, global_step=index)
        plt.close()
    ...
    def get_eval_datasets_and_ml_params(self, evaluate_on_train_datasets):
        ...
        for d in range(1, max_dimension + 1):
            ...
            val_datasets_sample_for_dim = self.val_dataset.get_n_datasets_with_input_dimension(
                self.n_eval_datasets_per_dim, d
            )
            ...
            if self.fixed_likelihood_variance:
                assert np.allclose(
                    val_datasets_sample_for_dim[0].observation_noise, np.sqrt(self.model_config.gp_variance)
                )
                assert np.allclose(
                    val_datasets_sample_for_dim[0].observation_noise,
                    self.gpytorch_model_config.initial_likelihood_noise,
                )
            ...
            if evaluate_on_train_datasets:
                ...
                train_datasets_sample_for_dim = self.train_dataset.get_n_datasets_with_input_dimension(
                    self.n_eval_datasets_per_dim, d
                )
                assert len(train_datasets_sample_for_dim) == self.n_eval_datasets_per_dim
                train_datasets_sample_ml_parameters = self.standard_GP_learning(
                    train_datasets_sample_for_dim, add_prior=False
                )
    ...
    def initialize_optimizer_and_scheduler(self, model):
        # Initialize optimizer
        ...
        if self.use_lr_scheduler:
            scheduler = torch.optim.lr_scheduler.OneCycleLR(
                optimizer, max_lr=self.learning_rate, steps_per_epoch=self.steps_per_epoch, epochs=self.num_epochs
            )
        else:
            scheduler = None
    ...
    def load_model_to_evaluator(self, device, model):
        model.to("cpu")
        state_dict = copy.deepcopy(model.state_dict())
        self.real_world_data_evaluator.load_model_from_state_dict(state_dict, self.model_config)
        model.to(device)
    ...
    def loss_function(
        ...
        if self.loss_function_type == LossFunctionType.NMLL:
            ...
            mse_kernel_params = (
                torch.sum(torch.square(batch_predicted_params - batch_gt_params) / n_kernel_params.unsqueeze(-1))
                / batch_size
            )
    ...
    def evaluate_prediction_on_dataset_samples(
        ...
        for d in range(1, max_dimension + 1):
            ...
            self.tensorboard_writer.add_scalar(
                "RMSE_diff_ml_{}/dim_{}".format(suffix, d),
                np.mean(rmses_amortized_val) - np.mean(rmses_ml_val),
                global_step=epoch_index,
            )

            self.tensorboard_writer.add_scalar(
                "NLL_diff_ml_{}/dim_{}".format(suffix, d),
                np.mean(nlls_amortized_val) - np.mean(nlls_ml_val),
                global_step=epoch_index,
            )
    ...
    def calculate_full_dataset_loss(self, model, data_loader, epoch_index, device):
        ...
        """
        assert isinstance(data_loader.dataset, DatasetOfDatasets) or isinstance(
            data_loader.dataset, RandomDatasetOfDatasets
        )
        with torch.no_grad():
    ...
    def add_prediction_plots(
        self, device, model, simulated_datasets_sample, ml_learned_parameters_sample, epoch_index, prefix
    ):
        """
        ...
        for i, simulated_dataset in enumerate(simulated_datasets_sample):
            ...
            kernel_gt = get_gpytorch_kernel_from_expression_and_state_dict(
                kernel_expression_gt, kernel_state_dict, wrap_in_addition=True
            )
    ...
    def print_shapes(
        self, X_padded, y_padded, N, size_mask, dim_mask, kernel_embeddings, kernel_mask, size_mask_kernel
    ):
    ...
    def save_config(
        self, training_config_dict, model_config_dict, dataset_folder, run_folder, generator_config_dict=None
    ):
    ...
    def save_checkpoint(
        self, model_state_dict, optimizer_state_dict, scheduler_state_dict, epoch_index, global_step, folder
    ):
        checkpoint = {}
