...
if __name__ == "__main__":
    ...
    plotter = Plotter(2)
    plotter.add_datapoints(X, y, "green", 0)
    plotter.add_datapoints(X, y, "green", 1)
    plotter.give_axes(0).set_title("Amortized Model")
    plotter.give_axes(1).set_title("regular Model")
    plotter.fig.set_size_inches(10.0, 10.0)
    plotter.fig.suptitle(
        cut_string(
            "Haloooooooooooooooooooooooooooooooooooooooooodfgdfgsahashooooooooooooooooasfdghdfshhoooooooooooooo", 100
        )
    )
    plotter.fig.tight_layout()
    plotter.show()
