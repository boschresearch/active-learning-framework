...
import torch
from alef.models.amortized_infer_structured_kernels.data_generators.base_data_generator import BaseDataGenerator
...
import numpy as np
from alef.models.amortized_infer_structured_kernels.utils.utils import convert_list_elements_to_int

from alef.utils.utils import write_dict_to_json, write_list_to_file
...
class DimWiseAdditiveKernelGenerator(BaseDataGenerator):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.weights_base_kernels = self.unnormalized_weights_base_kernels / np.sum(
            self.unnormalized_weights_base_kernels
        )
    ...
    def set_simulator_box_bounds(self, box_bound_lower: float, box_bound_upper: float):
        self.simulator.set_box_bounds(box_bound_lower, box_bound_upper)
    ...
    def generate_simulated_data(self, n_train: int, n_val: int, n_test: int, folder: str):
        ...
        write_dict_to_json(
            dimension_index_train, os.path.join(folder, DimWiseAdditiveKernelGenerator.dimension_index_file_train)
        )
        write_dict_to_json(
            dimension_index_val, os.path.join(folder, DimWiseAdditiveKernelGenerator.dimension_index_file_val)
        )
        write_dict_to_json(
            dimension_index_test, os.path.join(folder, DimWiseAdditiveKernelGenerator.dimension_index_file_test)
        )
            ...
            kernel_list_for_dim = convert_list_elements_to_int(
                list(
                    np.random.choice(
                        self.base_kerne_type_list,
                        kernel_len,
                        replace=self.replace_in_kernel_list,
                        p=self.weights_base_kernels,
                    )
                )
            )
