...
from alef.configs.prior_parameters import PRIOR_SETTING, PriorSettings
...
class SimulatedDataset:
    ...
    def __str__(self):
        ...
        describtion = (
            "simulated-dataset - n={} - dim={} - noise-std={} - mll={} - log-density={} - kernel_expression={}".format(
                self.n,
                self.input_dimension,
                self.observation_noise,
                self.mll_gt,
                self.log_posterior_density_gt,
                kernel_describtion,
            )
        )
        return describtion
...
class Simulator:
    ...
    def set_box_bounds(self, box_bound_lower: float, box_bound_upper: float):
        self.box_bound_lower = box_bound_lower
        self.box_bound_upper = box_bound_upper
    ...
    def create_sample(
        ...
        while not sampling_success:
            try:
                ...
                kernel_state_dict = get_hp_sample_from_prior_gpytorch_as_state_dict(
                    kernel_expression, wrap_in_addition=True
                )
                kernel = get_gpytorch_kernel_from_expression_and_state_dict(
                    kernel_expression, kernel_state_dict, wrap_in_addition=True
                )
