...
from alef.models.amortized_infer_structured_kernels.nn.module import (
    Attention,
    AttentionLayer,
    KernelEncodingLayer,
    KernelEncodingStandardLayer,
    Linear,
)
...
class KernelEncoder(BaseKernelEncoder):
    def __init__(
        self, input_dim: int, n_enc_dec_layer: int, hidden_dim: int, dataset_enc_dim: int, dropout_p: float, **kwargs
    ):
    ...
    def forward(self, kernel_embeddings, dataset_embeddings, kernel_mask, dim_mask):
        ...
        for layer in self.layers:
            kernel_embeddings, attns = layer(
                kernel_embeddings, kernel_embeddings, kernel_embeddings, dataset_embeddings, mask=kernel_mask
            )

        kernel_embeddings = kernel_embeddings.reshape(
            batch_size, max_dim, max_len_kernels, self.hidden_dim
        )  # B X D X N_k x d_h
...
class CrossAttentionKernelEncoder(BaseKernelEncoder):
        ...
        **kwargs,
        ...
        if use_standard_layers:
            ...
            self.layers_dec = nn.ModuleList(
                [
                    KernelEncodingStandardLayer(
                        self.hidden_dim, num_intermediate, self.dataset_enc_dim + self.hidden_dim, dropout_p
                    )
            ...
            self.layers_cross = nn.ModuleList(
                [Attention(self.hidden_dim, dropout_p) for i in range(self.n_cross_layer)]
            )
            self.layers_dec = nn.ModuleList(
                [
                    KernelEncodingLayer(self.hidden_dim, self.dataset_enc_dim + self.hidden_dim, dropout_p)
                    for i in range(self.n_dec_layer)
                ]
            )
    ...
    def forward(
        ...
        combined_dataset_cross_embedding = torch.cat(
            (original_dataset_embeddings, cross_embeddings), dim=2
        )  # B x D x (d_d + d_h)
        ...
        for layer in self.layers_dec:
            kernel_embeddings, attns = layer(
                ...
                kernel_embeddings,
                kernel_embeddings,
                combined_dataset_cross_embedding,
                mask=kernel_mask,
            )
