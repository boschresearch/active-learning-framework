...
from alef.models.amortized_infer_structured_kernels.nn.module import Attention, AttentionLayer, Linear
import numpy as np
...
class DatasetEncoder(nn.Module):
    def __init__(self, num_attentions1, num_attentions2, att1_hidden_dim, att2_hidden_dim, dropout_p):
        ...
        self.self_attentions1 = nn.ModuleList(
            [Attention(self.att1_hidden_dim, dropout_p) for tt in range(self.num_attentions1)]
        )

        self.self_attentions2 = nn.ModuleList(
            [Attention(self.att2_hidden_dim, dropout_p) for tt in range(self.num_attentions2)]
        )
    ...
    def forward(self, X_data_tr, F_data_tr, node_mask_tr, dim_mask, device=torch.device("cpu")):
        ...
        dim_encoder_input = dim_encoder_input.reshape(
            batch_size, max_dim, dim_encoder_input.shape[-1]
        )  # B X D X hidden_dim
...
class FullDimDatasetEncoder(nn.Module):
    ...
    def forward(self, X_data_tr, F_data_tr, node_mask_tr, dim_mask, device=torch.device("cpu")):
        ...
        X_data_expanded_padding = torch.zeros(
            batch_size, max_num_data, max_dim, padding_size, device=device
        )  # B x N x D x (INPUT_DIM - D)
...
class EnrichedDatasetEncoder(nn.Module):
    def __init__(
        ...
        if use_standard_layers:
            self.self_attentions1 = nn.ModuleList(
                [
                    AttentionLayer(self.att1_hidden_dim, dim_intermediate_bert, dropout_p)
            ...
            self.self_attentions2 = nn.ModuleList(
                [
                    AttentionLayer(self.att2_hidden_dim, dim_intermediate_bert, dropout_p)
            ...
            self.self_attentions3 = nn.ModuleList(
                [
                    AttentionLayer(self.att3_hidden_dim, dim_intermediate_bert, dropout_p)
                    for tt in range(self.num_attentions3)
                ]
            )

            self.self_attentions4 = nn.ModuleList(
                [
                    AttentionLayer(self.att4_hidden_dim, dim_intermediate_bert, dropout_p)
                    for tt in range(self.num_attentions4)
                ]
            )
        else:
            self.self_attentions1 = nn.ModuleList(
            ...
            self.self_attentions3 = nn.ModuleList(
                [Attention(self.att3_hidden_dim, dropout_p) for tt in range(self.num_attentions3)]
            )

            self.self_attentions4 = nn.ModuleList(
                [Attention(self.att4_hidden_dim, dropout_p) for tt in range(self.num_attentions4)]
            )
    ...
    def forward(self, X_data_tr, F_data_tr, node_mask, dim_mask, device=torch.device("cpu")):
        ...
        encoder_input_expanded = encoder_input.reshape(
            batch_size, max_dim, max_num_data, encoder_input.shape[-1]
        )  # B x D x N x hidden_dim
        ...
        datapoint_encoding = datapoint_encoding.reshape(
            -1, max_num_data, encoder_input.shape[-1]
        )  # (B x D) x N x hidden_dim
...
def get_padded_dataset_and_masks(X_list: List[np.array], y_list: List[np.array]) -> Tuple[np.array, np.array, np.array]:
    ...
    size_mask_kernel = np.stack(
        [
            np.pad(
                np.ones((size, size)), ((0, size_paddings[i]), (0, size_paddings[i])), "constant", constant_values=0.0
            )
...
if __name__ == "__main__":
    ...
    print(size_mask[0])
    print(dim_mask[0])
    print(size_mask_kernel)
    print(size_mask_kernel.shape)
    print(N[0])
    y_tensor = torch.from_numpy(y_padded).float()
    X_tensor = torch.from_numpy(X_padded).float()
    node_mask_tensor = torch.from_numpy(size_mask).float()
    dim_mask_tensor = torch.from_numpy(dim_mask).float()
    encoder = DatasetEncoder(num_attentions1=2, num_attentions2=2, att1_hidden_dim=16, att2_hidden_dim=16)
    output = encoder.forward(X_tensor, y_tensor, node_mask_tensor, dim_mask_tensor)
    print(output)
    print(output.shape)
