...
import numpy as np
from alef.models.amortized_infer_structured_kernels.gp.rbf_kernel_wrapper import BatchedRBFKernelWrapper
from alef.models.amortized_infer_structured_kernels.nn.dataset_encoder import (
    DatasetEncoder,
    get_padded_dataset_and_masks,
)
from alef.models.amortized_infer_structured_kernels.nn.kernel_encoder import KernelEncoder
...
from alef.models.amortized_infer_structured_kernels.gp.utils import (
    GP_noise,
    cal_marg_likelihood,
    cal_marg_likelihood_batch_noise,
)
from alef.models.amortized_infer_structured_kernels.nn.noise_predictor_head import (
...
from alef.models.amortized_infer_structured_kernels.gp.base_kernels import flatten
...
class BasicDimWiseAdditiveAmortizedInferenceModel(nn.Module):
    ...
    @abstractmethod
    def get_num_predicted_params(
        self, kernel_list: List[List[List[BaseKernelTypes]]], device=torch.device("cpu")
    ) -> torch.Tensor:
        """
    ...
    def predict_head(
        ...
        # assert isinstance(kernel_list[0][0], BaseKernelTypes) or isinstance(kernel_list[0][0], int)
        X_test = torch.from_numpy(X_test).to(torch.get_default_dtype()).to(device)
        ...
        mu_test, covar_test = GP_noise(
            y_unsqueezed.squeeze(0),
            K_train.squeeze(0),
            K_train_test.squeeze(0),
            K_test.squeeze(0),
            noise_variance,
            device,
        )
...
class DimWiseAdditiveKernelsAmortizedInferenceModel(BasicDimWiseAdditiveAmortizedInferenceModel):
    ...
    def forward(
        ...
        dataset_embedding = self.dataset_encoder.forward(
            X_data, Y_data, size_mask, dim_mask, device
        )  # B X D X hidden_dim_1
        ...
        mll, mll_success = cal_marg_likelihood(
            K, Y_data.unsqueeze(-1), self.epsilon, size_mask_kernel, 1.0 - size_mask, N_data, device
        )
        if mll_success:
            nmll = -1.0 * mll
...
class DimWiseAdditiveKernelWithNoiseAmortizedModel(BasicDimWiseAdditiveAmortizedInferenceModel):
    ...
    def forward_ensemble(
        ...
        K = self.kernel_wrapper.forward(
            X_data_extended, X_data_extended, kernel_embeddings, kernel_list
        )  # B_ens x N x N
        ...
        mll, mll_success = cal_marg_likelihood_batch_noise(
            K,
            Y_data_extended.unsqueeze(-1),
            noise_variances,
            size_mask_kernel_extended,
            1.0 - size_mask_extended,
            N_data_extended,
            device,
        )  # B
        # chol in mll might fail due to numerical instabilites - some upstream methods might not use the mll such as predict --> we dont throw an error here but push the information upstream
    ...
    def warm_start(
        ...
        if only_learn_noise_variance:
            ...
            params = flatten(untransformed_kernel_params) + [noise_variances_untransformed]
        optimizer = torch.optim.Adam(params, lr=learning_rate)
        for i in range(0, num_steps):
            optimizer.zero_grad()
            K = self.kernel_wrapper.forward(
                X_data, X_data, kernel_embeddings, kernel_list, untransformed_kernel_params
            )  # B x N x N
...
class ARDRBFOnlyAmortizedInferenceModel(BasicDimWiseAdditiveAmortizedInferenceModel):
    ...
    def forward(
        ...
        dataset_embedding = self.dataset_encoder.forward(
            X_data, Y_data, size_mask, dim_mask, device
        )  # B X D X kernel_wrapper_hidden_dim
...
def to_input_tensor_of_dim_wise_additive(kernel_list, device, X_list, y_list):
    ...
    kernel_embeddings = torch.from_numpy(kernel_embeddings).to(torch.get_default_dtype()).to(device)
    kernel_mask = torch.from_numpy(kernel_mask).to(torch.get_default_dtype()).to(device)
    X_padded = torch.from_numpy(X_padded).to(torch.get_default_dtype()).to(device)
    y_padded = torch.from_numpy(y_padded).to(torch.get_default_dtype()).to(device)
    size_mask = torch.from_numpy(size_mask).to(torch.get_default_dtype()).to(device)
    dim_mask = torch.from_numpy(dim_mask).to(torch.get_default_dtype()).to(device)
    size_mask_kernel = torch.from_numpy(size_mask_kernel).to(torch.get_default_dtype()).to(device)
    N = torch.from_numpy(N).to(torch.get_default_dtype()).to(device)
    return kernel_mask, kernel_embeddings, X_padded, y_padded, size_mask, dim_mask, N, size_mask_kernel


if __name__ == "__main__":
    dataset_encoder = DatasetEncoder(3, 3, 128, 128)
    kernel_encoder = KernelEncoder(6, 5, 12, 128)
    noise_predictor = NoiseVariancePredictorHead([10], 12, 1e-4)
    amor_model = DimWiseAdditiveKernelWithNoiseAmortizedModel(dataset_encoder, kernel_encoder, noise_predictor, True)
    # amor_model = DimWiseAdditiveKernelsAmortizedInferenceModel(dataset_encoder, kernel_encoder, 1e-3)
    _, _, X_padded, y_padded, N, size_mask, dim_mask, kernel_embeddings, kernel_mask, size_mask_kernel, kernel_list = (
        get_test_inputs(5, 10, 4, 4)
    )
    _, _, nmll, _, noise_variances, _, _, _ = amor_model.forward(
        X_data=X_padded,
        Y_data=y_padded,
        N_data=N,
        size_mask=size_mask,
        dim_mask=dim_mask,
        kernel_embeddings=kernel_embeddings,
        kernel_mask_enc=kernel_mask,
        size_mask_kernel=size_mask_kernel,
        kernel_list=kernel_list,
    )
    print(nmll)
