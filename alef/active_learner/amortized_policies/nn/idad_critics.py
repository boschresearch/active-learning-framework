...
https://github.com/desi-ivanova/idad/blob/main/neural/critics.py
...
class CriticJointNetwork(nn.Module):
    ...
    def __init__(self, history_encoder_network, latent_encoder_network, head_layer=None):
        super().__init__()
        ...
        if head_layer is not None:
            ...
            input_dim = latent_encoder_network.encoding_dim + history_encoder_network.encoding_dim
...
class CriticBA(nn.Module):
    ...
    def forward(self, latent, *design_obs_pairs):
        ...
        log_probs_q = torch.distributions.Normal(loc=mean, scale=sd).log_prob(latent_flat).sum(axis=-1)
