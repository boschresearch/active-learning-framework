    ...
    def __init__(
        ...
        **kwargs,
        ...
        self.attn_layers = nn.ModuleList(
            [get_attender(attention, x_dim, x_dim, x_dim, **kwargs) for _ in range(n_attn_layers)]
        )
...
def get_attender(attention, kq_size, value_size, out_size, **kwargs):
    ...
    if attention == "multiplicative":
        ...
        attender = DistanceAttender(kq_size, value_size, out_size, is_weight=True, p=1, **kwargs)
...
class DotAttender(BaseAttender):
    ...
    def score(self, keys, queries):
        ...
        logits = torch.einsum("{},{}->bqk".format(keys_shape, queries_shape), keys, queries)
...
class MultiheadAttender(nn.Module):
    ...
    def __init__(
        ...
        self.value_size = value_size
        self.out_size = out_size
        self.post_processor = nn.Linear(value_size, out_size) if is_post_process or value_size != out_size else None
    ...
    def _make_multiheaded(self, kvq, head_size):
        ...
        kvq = kvq.permute(2, 0, 1, 3).contiguous().view(batch_size * self.n_heads, -1, head_size)
        return kvq
    ...
    def _concatenate_multiheads(self, kvq, head_size):
        ...
        kvq = kvq.permute(1, 2, 0, 3).contiguous().view(batch_size, -1, self.n_heads * head_size)
        return kvq
