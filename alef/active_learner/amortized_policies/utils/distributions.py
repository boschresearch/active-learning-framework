...
https://github.com/ae-foster/dad/blob/main/extra_distributions/truncated_normal.py
...
class LowerTruncatedNormal(TorchDistribution, ExponentialFamily):
    ...
    def cdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        return (self._normal_cdf(value) - self._normal_cdf(self.truncation)).clamp(min=0.0)

    def icdf(self, value):
        if self._validate_args:
            self._validate_sample(value)
        cdf_truncation = self._normal_cdf(self.truncation)
        rescaled_value = cdf_truncation + (1.0 - cdf_truncation) * value
